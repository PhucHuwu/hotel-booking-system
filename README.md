...
| :-: | -------------- | ---------- | ---------------------------------------------- | ------------------------------- |
|  1  | Lương Tuấn Anh | B22DCCN021 | [@n0xgg04](https://github.com/n0xgg04)         | Trưởng nhóm · Backend · Infra   |
...
```mermaid
flowchart TD
  CLIENT[Client] -->|POST /bookings| BOOKING[BookingService]

  BOOKING --> LOCK[Redis SETNX lock 10s]
  LOCK -->|Da lock| CONFLICT[ConflictException]
  LOCK -->|Thanh cong| OVERLAP[Overlap check]
  OVERLAP --> TX[DB Transaction\nBooking PENDING\nRoom RESERVED\nOutboxEvent]
  TX --> PUB[Publish booking.created\nbooking.exchange]

  PUB --> PAYMENT[PaymentService\nconsumes booking.created\nCreates Payment record\nGenerates VNPay URL]

  PAYMENT -->|payment.success| CONFIRM[BookingSaga confirmBooking\nRoom AVAILABLE\nBooking CONFIRMED\nPublish booking.confirmed]
  PAYMENT -->|payment.failed| CANCEL[BookingSaga cancelBooking\nCompensating Tx\nRoom AVAILABLE\nBooking CANCELLED\nPublish booking.cancelled]

  CONFIRM --> NOTI[NotificationConsumer\nSend email via SMTP\nLog Notification table]
  CANCEL --> NOTI
```
...
```mermaid
flowchart TB
  MAIN[main\nproduction-ready\nchi merge tu develop]
  DEVELOP[develop\nintegration branch]

  INFRA[feat/infra\nn0xgg04\nMonorepo setup, Prisma schema, Docker]
  AUTH[feat/auth\nn0xgg04\nUC01 Auth + JWT + RBAC]
  ROOMS[feat/room-management\nPhucHuwu\nUC11+UC12 Rooms + Pricing]
  BOOKPAY[feat/booking-payment\nPhucHuwu\nUC03+UC04+UC16 Booking + Saga + Payment]
  STAFF[feat/staff-operations\nn0xgg04\nUC07-UC10 Room Map + Check-in/out]
  REPORT[feat/reporting\nn0xgg04\nUC14+UC15 Reports + Notifications]
  FE_CUS[feat/frontend-customer\nluongdat2k4\nCustomer portal]
  FE_ADM[feat/frontend-admin\nbngcVu\nAdmin + Staff portal]
...
erDiagram
  ...
  ROOM_TYPE ||--o{ ROOM : "roomTypeId"
  ROOM_TYPE ||--o{ PRICING_RULE : "roomTypeId"
  ROOM ||--o{ BOOKING : "roomId"
  BOOKING ||--o{ BOOKING_ADDON : "bookingId"
  BOOKING ||--o| REVIEW : "bookingId"
  BOOKING ||--o{ PAYMENT : "bookingId"
  BOOKING ||--o{ OUTBOX_EVENT : "aggregateId -> Booking.id"
